...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const selectedSubCategory = document.getElementById('selected-sub-category');
        ...
        .then(categories => {
            categories.forEach(cat => {
                ...
                li.addEventListener('click', () => {
                    // 서브카테고리 가져오기
                    fetch(`/get_subcategories/${cat.id}`)
                        .then(res => res.json())
                        .then(subCats => {
                                    ...
                                    fetch(`/get_products/${sub.id}`)
                                        ...
                                        .then(products => {
                                            ...
                                            products.forEach(p => {
                                                ...
                                                card.innerHTML = `<strong>${p.name}</strong><p>${p.price}원</p>`;
                                                productListDiv.appendChild(card);
                                            });
                                            productRight.style.display = 'block';
                                            selectedSubCategory.textContent = sub.name;
                                        });
                                });
                            });
